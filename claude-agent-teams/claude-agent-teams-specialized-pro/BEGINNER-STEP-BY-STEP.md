...
### 풀 파이프라인 (처음부터 끝까지 한 번에)
```text
/full-pipeline
중소형주 모멘텀 전략을 연구부터 코드 구현, 최종 검증까지 한 번에 진행해줘.
```
`/full-pipeline`은 작업 종류를 자동으로 판단해서 필요한 팀을 순서대로 실행합니다.
예: 퀀트 → quant-research → quant-build → audit-final 순서로 진행.
